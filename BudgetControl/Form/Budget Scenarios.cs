...
namespace BudgetControl
{
    public partial class Budget_Scenarios : Form
    {
        ...
        private void dataGridView1_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (e.ColumnIndex == dataGridView1.Columns["ColBudgetCode"].Index)
            {
                tempBGCode = dataGridView1.Rows[e.RowIndex].Cells["ColBudgetCode"].Value.ToString();
            }
        }

        private void dataGridView1_RowValidating(object sender, DataGridViewCellCancelEventArgs e)
        {
            if (dataGridView1.Rows[e.RowIndex].Cells["ColBudgetPeriod"].Value == null)
                return;
        ...
        {
            if (e.ColumnIndex == dataGridView1.Columns["ColBudgetCode"].Index)
            {
                if (dataGridView1.Rows[e.RowIndex].Cells["ColReviseNo"].Value.ToIntorDefault() != -1)
                {
                    if (tempBGCode != dataGridView1.Rows[e.RowIndex].Cells["ColBudgetCode"].Value.ToString())
                    {
                        e.Cancel = true;
                        MessageBox.Show("ไม่สามารถทำการแก้ไข Budget Code ได้เนื่องจากมีการนำไปใช้งานแล้ว", "ตรวจสอบ", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    }
                }
            }
        }
